...
public final class InternetDomainName {
    ...
    private static final Joiner DOT_JOINER = Joiner.m1485on((char) FilenameUtils.EXTENSION_SEPARATOR);
    private static final String DOT_REGEX = "\\.";
    private static final Splitter DOT_SPLITTER = Splitter.m1498on((char) FilenameUtils.EXTENSION_SEPARATOR);
    private static final int MAX_DOMAIN_PART_LENGTH = 63;
    private static final int MAX_LENGTH = 253;
    private static final int MAX_PARTS = 127;
    private static final int NO_PUBLIC_SUFFIX_FOUND = -1;
    private static final CharMatcher PART_CHAR_MATCHER = CharMatcher.JAVA_LETTER_OR_DIGIT.mo14368or(DASH_MATCHER);
    ...
    InternetDomainName(String name2) {
        boolean z;
        String name3 = Ascii.toLowerCase(DOTS_MATCHER.replaceFrom((CharSequence) name2, (char) FilenameUtils.EXTENSION_SEPARATOR));
        if (name3.endsWith(".")) {
        ...
        Preconditions.checkArgument(z, "Domain has too many parts: '%s'", name3);
    ...
    private static boolean validateSyntax(List<String> parts2) {
        ...
        if (!validatePart((String) parts2.get(lastIndex), true)) {
            return false;
        }
        for (int i = 0; i < lastIndex; i++) {
            if (!validatePart((String) parts2.get(i), false)) {
                return false;
            }
        }
        return true;
