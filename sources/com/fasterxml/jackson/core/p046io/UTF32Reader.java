...
public class UTF32Reader extends BaseReader {
    ...
    public int read(char[] cbuf, int start, int len) throws IOException {
        ...
        while (true) {
            ...
            int ptr = this._ptr;
            if (this._bigEndian) {
                ch = (this._buffer[ptr] << Ascii.CAN) | ((this._buffer[ptr + 1] & 255) << Ascii.DLE) | ((this._buffer[ptr + 2] & 255) << 8) | (this._buffer[ptr + 3] & 255);
            } else {
                ch = (this._buffer[ptr] & 255) | ((this._buffer[ptr + 1] & 255) << 8) | ((this._buffer[ptr + 2] & 255) << Ascii.DLE) | (this._buffer[ptr + 3] << Ascii.CAN);
            }
            this._ptr += 4;
            if (ch > 65535) {
                if (ch > 1114111) {
