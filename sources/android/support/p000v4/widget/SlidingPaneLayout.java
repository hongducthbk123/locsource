        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (child.getVisibility() == 8) {
                ...
                if (lp.height == -2) {
                    ...
                    childHeightSpec3 = MeasureSpec.makeMeasureSpec(maxLayoutHeight, Ints.MAX_POWER_OF_TWO);
                } else {
                    childHeightSpec3 = MeasureSpec.makeMeasureSpec(lp.height, Ints.MAX_POWER_OF_TWO);
                }
                ...
                if (child2.getVisibility() != 8) {
                    ...
                    if (child2.getVisibility() != 8) {
                        ...
                        if (!canSlide || child2 == this.mSlideableView) {
                            if (lp2.weight > 0.0f) {
                                if (lp2.width != 0) {
                                    childHeightSpec = MeasureSpec.makeMeasureSpec(child2.getMeasuredHeight(), Ints.MAX_POWER_OF_TWO);
                                    ...
                                    childHeightSpec = MeasureSpec.makeMeasureSpec(lp2.height, Ints.MAX_POWER_OF_TWO);
                                }
                                if (canSlide) {
                                    int newWidth = widthAvailable - (lp2.leftMargin + lp2.rightMargin);
                                    int childWidthSpec2 = MeasureSpec.makeMeasureSpec(newWidth, Ints.MAX_POWER_OF_TWO);
                                    if (measuredWidth != newWidth) {
                                        child2.measure(childWidthSpec2, childHeightSpec);
                                    }
                                } else {
                                    child2.measure(MeasureSpec.makeMeasureSpec(measuredWidth + ((int) ((lp2.weight * ((float) Math.max(0, widthRemaining))) / weightSum)), Ints.MAX_POWER_OF_TWO), childHeightSpec);
                                }
                            }
                        } else if (lp2.width < 0 && (measuredWidth > fixedPanelWidthLimit || lp2.weight > 0.0f)) {
                            if (!skippedFirstPass) {
                                childHeightSpec2 = MeasureSpec.makeMeasureSpec(child2.getMeasuredHeight(), Ints.MAX_POWER_OF_TWO);
                            } else if (lp2.height == -2) {
                                childHeightSpec2 = MeasureSpec.makeMeasureSpec(maxLayoutHeight, Integer.MIN_VALUE);
                            } else if (lp2.height == -1) {
                                childHeightSpec2 = MeasureSpec.makeMeasureSpec(maxLayoutHeight, Ints.MAX_POWER_OF_TWO);
                            } else {
                                childHeightSpec2 = MeasureSpec.makeMeasureSpec(lp2.height, Ints.MAX_POWER_OF_TWO);
                            }
                            child2.measure(MeasureSpec.makeMeasureSpec(fixedPanelWidthLimit, Ints.MAX_POWER_OF_TWO), childHeightSpec2);
                        }
                    }
                }
            }
