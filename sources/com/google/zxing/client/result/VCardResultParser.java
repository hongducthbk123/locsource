...
import com.google.android.vending.expansion.downloader.impl.DownloadsDB.DownloadColumns;
...
public final class VCardResultParser extends ResultParser {
    ...
    public AddressBookParsedResult parse(Result result) {
        ...
        List<List<String>> names = matchVCardPrefixedField(DownloadColumns.FILENAME, rawText, true, false);
        if (names == null) {
            ...
            if (i < 0) {
                ...
                if (quotedPrintable) {
                    ...
                    if (parseFieldDivider) {
                        element = UNESCAPED_SEMICOLONS.matcher(element).replaceAll(IOUtils.LINE_SEPARATOR_UNIX).trim();
                    }
                } else {
                    if (parseFieldDivider) {
                        element2 = UNESCAPED_SEMICOLONS.matcher(element2).replaceAll(IOUtils.LINE_SEPARATOR_UNIX).trim();
                    }
                    element = VCARD_ESCAPES.matcher(NEWLINE_ESCAPE.matcher(CR_LF_SPACE_TAB.matcher(element2).replaceAll("")).replaceAll(IOUtils.LINE_SEPARATOR_UNIX)).replaceAll("$1");
                }
                if (metadata == null) {
