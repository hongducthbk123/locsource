...
import com.google.common.p047io.ByteStreams;
...
public final class InetAddresses {
    ...
    private static byte parseOctet(String ipPart) {
        ...
        if (octet <= 255 && (!ipPart.startsWith(AppEventsConstants.EVENT_PARAM_VALUE_NO) || ipPart.length() <= 1)) {
            return (byte) octet;
        }
    ...
    public static String toAddrString(InetAddress ip) {
        ...
        for (int i = 0; i < hextets.length; i++) {
            hextets[i] = Ints.fromBytes(0, 0, bytes[i * 2], bytes[(i * 2) + 1]);
        }
