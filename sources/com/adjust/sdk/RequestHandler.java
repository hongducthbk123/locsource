...
public class RequestHandler implements IRequestHandler {
    ...
    public void sendI(ActivityPackage activityPackage, int queueSize) {
        ...
        try {
            ResponseData responseData = UtilNetworking.createPOSTHttpsURLConnection(url + activityPackage.getPath(), activityPackage, queueSize);
    ...
    private void closePackageI(ActivityPackage activityPackage, String message, Throwable throwable) {
        String finalMessage = Util.formatString("%s. (%s) Will retry later", activityPackage.getFailureMessage(), Util.getReasonString(message, throwable));
    ...
    private void sendNextPackageI(ActivityPackage activityPackage, String message, Throwable throwable) {
        String finalMessage = Util.formatString("%s. (%s)", activityPackage.getFailureMessage(), Util.getReasonString(message, throwable));
        this.logger.error(finalMessage, new Object[0]);
        ResponseData responseData = ResponseData.buildResponseData(activityPackage);
        responseData.message = finalMessage;
        IPackageHandler packageHandler = (IPackageHandler) this.packageHandlerWeakRef.get();
