...
public class PathInterpolatorCompat implements Interpolator {
    ...
    private void initPath(Path path) {
        ...
        while (i2 < numPoints) {
            ...
            float x = this.f3mX[componentIndex];
            if (x < prevX) {
    ...
    public float getInterpolation(float t) {
        ...
        float xRange = this.f3mX[endIndex] - this.f3mX[startIndex];
        if (xRange == 0.0f) {
            return this.f4mY[startIndex];
        }
        float fraction = (t - this.f3mX[startIndex]) / xRange;
        float startY = this.f4mY[startIndex];
        return ((this.f4mY[endIndex] - startY) * fraction) + startY;
    }
}
