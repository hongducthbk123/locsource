...
class LicenseValidator {
    ...
    public void verify(PublicKey publicKey, int responseCode, String signedData, String signature) {
        ...
        if (responseCode == 0 || responseCode == 1 || responseCode == 2) {
            try {
                Signature limiterResponse = Signature.getInstance(SIGNATURE_ALGORITHM);
                limiterResponse.initVerify(publicKey);
                limiterResponse.update(signedData.getBytes());
                if (!limiterResponse.verify(Base64.decode(signature))) {
