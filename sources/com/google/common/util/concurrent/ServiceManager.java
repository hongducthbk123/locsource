...
public final class ServiceManager {
        ...
        public void awaitHealthy(long timeout, TimeUnit unit) throws TimeoutException {
            ...
            try {
                if (!this.monitor.waitForUninterruptibly(this.awaitHealthGuard, timeout, unit)) {
                    throw new TimeoutException("Timeout waiting for the services to become healthy. The following services have not started: " + Multimaps.filterKeys(this.servicesByState, Predicates.m1491in(ImmutableSet.m1551of(State.NEW, State.STARTING))));
                }
                checkHealthy();
            } finally {
                this.monitor.leave();
        ...
        public void awaitStopped(long timeout, TimeUnit unit) throws TimeoutException {
            ...
            try {
                if (!this.monitor.waitForUninterruptibly(this.stoppedGuard, timeout, unit)) {
                    throw new TimeoutException("Timeout waiting for the services to stop. The following services have not stopped: " + Multimaps.filterKeys(this.servicesByState, Predicates.not(Predicates.m1491in(ImmutableSet.m1551of(State.TERMINATED, State.FAILED)))));
                }
            } finally {
                this.monitor.leave();
            }
    ...
    public ServiceManager(Iterable<? extends Service> services2) {
        ...
        if (copy.isEmpty()) {
            ...
            copy = ImmutableList.m1512of(new NoOpService());
        }
