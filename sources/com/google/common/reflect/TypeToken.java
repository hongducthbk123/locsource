...
public abstract class TypeToken<T> extends TypeCapture<T> implements Serializable {
    ...
    private static abstract class TypeCollector<K> {
        ...
        public final ImmutableList<K> collectTypes(K type) {
            return collectTypes((Iterable<? extends K>) ImmutableList.m1512of(type));
        }
    ...
    protected TypeToken(Class<?> declaringClass) {
        ...
        if (captured instanceof Class) {
            ...
            this.runtimeType = m1604of(declaringClass).resolveType(captured).runtimeType;
        }
    }
    ...
    /* renamed from: of */
    public static <T> TypeToken<T> m1604of(Class<T> type) {
        return new SimpleTypeToken(type);
    }

    /* renamed from: of */
    public static TypeToken<?> m1605of(Type type) {
        return new SimpleTypeToken(type);
    }
    ...
    public final <X> TypeToken<T> where(TypeParameter<X> typeParam, TypeToken<X> typeArg) {
        return new SimpleTypeToken(new TypeResolver().where(ImmutableMap.m1531of(new TypeVariableKey(typeParam.typeVariable), typeArg.runtimeType)).resolveType(this.runtimeType));
    }

    public final <X> TypeToken<T> where(TypeParameter<X> typeParam, Class<X> typeArg) {
        return where(typeParam, m1604of(typeArg));
    }
    ...
    public final TypeToken<?> resolveType(Type type) {
        ...
        return m1605of(resolver.resolveType(type));
    }
    ...
    private ImmutableList<TypeToken<? super T>> boundsAsInterfaces(Type[] bounds) {
        ...
        for (Type bound : bounds) {
            TypeToken<? super T> boundType = m1605of(bound);
    ...
    public final TypeToken<? extends T> getSubtype(Class<?> subclass) {
        ...
        return m1605of(resolveTypeArgsForSubclass(subclass));
    }
    ...
    public final TypeToken<T> wrap() {
        if (isPrimitive()) {
            return m1604of(Primitives.wrap((Class) this.runtimeType));
        }
        return this;
    }
    ...
    public final TypeToken<T> unwrap() {
        if (isWrapper()) {
            return m1604of(Primitives.unwrap((Class) this.runtimeType));
        }
        return this;
    }
    ...
    public final TypeToken<?> getComponentType() {
        ...
        return m1605of(componentType);
    }

    public final Invokable<T, Object> method(Method method) {
        Preconditions.checkArgument(m1604of(method.getDeclaringClass()).isAssignableFrom(this), "%s not declared by %s", method, this);
    ...
    public final Invokable<T, T> constructor(Constructor<?> constructor) {
        ...
        return new ConstructorInvokable<T>(constructor) {
            ...
            public String toString() {
                return getOwnerType() + "(" + Joiner.m1486on(", ").join((Object[]) getGenericParameterTypes()) + ")";
            }
        };
    }
    ...
    public Object writeReplace() {
        return m1605of(new TypeResolver().resolveType(this.runtimeType));
    }
    ...
    private static boolean isAssignableToParameterizedType(Type from, ParameterizedType to) {
        ...
        TypeToken<?> fromTypeToken = m1605of(from);
    ...
    static <T> TypeToken<? extends T> toGenericType(Class<T> cls) {
        if (cls.isArray()) {
            return m1605of(Types.newArrayType(toGenericType(cls.getComponentType()).runtimeType));
        }
        TypeVariable<Class<T>>[] typeParams = cls.getTypeParameters();
        if (typeParams.length > 0) {
            return m1605of((Type) Types.newParameterizedType(cls, typeParams));
        }
        return m1604of(cls);
    }
    ...
    private TypeToken<? super T> getSupertypeFromUpperBounds(Class<? super T> supertype, Type[] upperBounds) {
        for (Type upperBound : upperBounds) {
            TypeToken<? super T> bound = m1605of(upperBound);
            if (m1604of(supertype).isAssignableFrom(bound)) {
    ...
    private TypeToken<? extends T> getSubtypeFromLowerBounds(Class<?> subclass, Type[] lowerBounds) {
        ...
        if (0 < arr$.length) {
            return m1605of(arr$[0]).getSubtype(subclass);
        }
        throw new IllegalArgumentException(subclass + " isn't a subclass of " + this);
    }

    private TypeToken<? super T> getArraySupertype(Class<? super T> supertype) {
        return m1605of(newArrayClassOrGenericArrayType(((TypeToken) Preconditions.checkNotNull(getComponentType(), "%s isn't a super type of %s", supertype, this)).getSupertype(supertype.getComponentType()).runtimeType));
    }

    private TypeToken<? extends T> getArraySubtype(Class<?> subclass) {
        return m1605of(newArrayClassOrGenericArrayType(getComponentType().getSubtype(subclass.getComponentType()).runtimeType));
    }
