...
public class Draft_10 extends Draft {
    ...
    public ByteBuffer createBinaryFrame(Framedata framedata) {
        ...
        if ($assertionsDisabled || payloadlengthbytes.length == sizebytes) {
            if (sizebytes == 1) {
                buf.put((byte) ((mask ? UnsignedBytes.MAX_POWER_OF_TWO : 0) | payloadlengthbytes[0]));
            } else if (sizebytes == 2) {
    ...
    public Framedata translateSingleFrame(ByteBuffer buffer) throws IncompleteException, InvalidDataException {
        ...
        byte b1 = buffer.get();
        boolean FIN = (b1 >> 8) != 0;
        byte rsv = (byte) ((b1 & Ascii.DEL) >> 4);
        if (rsv != 0) {
            throw new InvalidFrameException("bad rsv " + rsv);
        }
        byte b2 = buffer.get();
        boolean MASK = (b2 & UnsignedBytes.MAX_POWER_OF_TWO) != 0;
        int payloadlength = (byte) (b2 & Ascii.DEL);
        Opcode optcode = toOpcode((byte) (b1 & Ascii.f977SI));
            ...
            if (payloadlength < 0 || payloadlength > 125) {
                if (optcode == Opcode.PING || optcode == Opcode.PONG || optcode == Opcode.CLOSING) {
                    ...
                    if (maxpacketsize < realpacketsize) {
                        IncompleteException incompleteException2 = new IncompleteException(realpacketsize);
                    ...
                    realpacketsize = 2 + 8;
                    if (maxpacketsize < realpacketsize) {
                        IncompleteException incompleteException3 = new IncompleteException(realpacketsize);
