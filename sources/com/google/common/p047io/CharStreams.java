...
public final class CharStreams {
    ...
    public static <R extends Readable & Closeable> List<String> readLines(InputSupplier<R> supplier) throws IOException {
        ...
        try {
            List<String> readLines = readLines((Readable) (Readable) closer.register((Closeable) supplier.getInput()));
            closer.close();
            return readLines;
        } catch (Throwable th) {
            closer.close();
    ...
    public static <R extends Readable & Closeable, T> T readLines(InputSupplier<R> supplier, LineProcessor<T> callback) throws IOException {
        ...
        Closer closer = Closer.create();
        try {
            T readLines = readLines((Readable) (Readable) closer.register((Closeable) supplier.getInput()), callback);
            closer.close();
            return readLines;
        } catch (Throwable th) {
            closer.close();
    ...
    public static InputSupplier<Reader> join(Iterable<? extends InputSupplier<? extends Reader>> suppliers) {
        ...
        return asInputSupplier(CharSource.concat(Iterables.transform(suppliers, new Function<InputSupplier<? extends Reader>, CharSource>() {
