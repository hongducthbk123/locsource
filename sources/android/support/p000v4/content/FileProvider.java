    ...
    static class SimplePathStrategy implements PathStrategy {
        ...
        public Uri getUriForFile(File file) {
            ...
            try {
                ...
                if (rootPath2.endsWith(HttpUtils.PATHS_SEPARATOR)) {
                    path = path2.substring(rootPath2.length());
                } else {
                    path = path2.substring(rootPath2.length() + 1);
                }
                return new Builder().scheme("content").authority(this.mAuthority).encodedPath(Uri.encode((String) mostSpecific.getKey()) + IOUtils.DIR_SEPARATOR_UNIX + Uri.encode(path, HttpUtils.PATHS_SEPARATOR)).build();
            } catch (IOException e) {
