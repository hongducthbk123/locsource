...
class StringMaker {
    ...
    public String makeModifiersString(int modifiers) {
        ...
        return new StringBuffer().append(str).append(MinimalPrettyPrinter.DEFAULT_ROOT_VALUE_SEPARATOR).toString();
    }
    ...
    public String makeTypeName(Class type, String typeName, boolean shortName) {
        ...
        if (type.isArray()) {
            ...
            return stripPackageName(typeName).replace('$', FilenameUtils.EXTENSION_SEPARATOR);
        } else {
            return typeName.replace('$', FilenameUtils.EXTENSION_SEPARATOR);
        }
    }
