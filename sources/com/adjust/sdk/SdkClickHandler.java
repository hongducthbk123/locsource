...
public class SdkClickHandler implements ISdkClickHandler {
    ...
    public void sendNextSdkClickI() {
        if (!this.paused && !this.packageQueue.isEmpty()) {
            ...
            this.logger.verbose("Waiting for %s seconds before retrying sdk_click for the %d time", Util.SecondsDisplayFormat.format(((double) waitTimeMilliSeconds) / MILLISECONDS_TO_SECONDS_DIVISOR), Integer.valueOf(retries));
    ...
    public void sendSdkClickI(ActivityPackage sdkClickPackage) {
        ...
        if (isReftag) {
            SharedPreferencesManager sharedPreferencesManager = new SharedPreferencesManager(activityHandler.getContext());
            if (sharedPreferencesManager.getRawReferrer(rawReferrerString, sdkClickPackage.getClickTimeInMilliseconds()) == null) {
                return;
            }
        }
    ...
    private void retrySendingI(ActivityPackage sdkClickPackage) {
        this.logger.error("Retrying sdk_click package for the %d time", Integer.valueOf(sdkClickPackage.increaseRetries()));
    ...
    private void logErrorMessageI(ActivityPackage sdkClickPackage, String message, Throwable throwable) {
        this.logger.error(Util.formatString("%s. (%s)", sdkClickPackage.getFailureMessage(), Util.getReasonString(message, throwable)), new Object[0]);
    }
}
